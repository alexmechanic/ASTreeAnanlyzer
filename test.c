...
static void rtl8139_start_thread(struct dev_private *tp);
...
static void __rtl8139_cleanup_dev (struct net_device *dev) {
	struct dev_private *tp = netdev_priv(dev);
	struct pci_dev *pdev;
...
static int rtl8139_set_features(struct net_device *dev, netdev_features_t features) {
	...
	if (changed & NETIF_F_RXALL) {
		...
		if (features & NETIF_F_RXALL)
			...
			rx_mode &= ~(AcceptErr | AcceptRunt);
		tp->rx_config = dev_rx_config | rx_mode;
...
static void rtl8139_hw_start (struct net_device *dev) {
	...
	tp->rx_config = dev_rx_config | AcceptBroadcast | AcceptMyPhys;
	RTL_W32 (RxConfig, tp->rx_config);
	RTL_W32 (TxConfig, dev_tx_config);
	...
	RTL_W16 (IntrMask, dev_intr_mask);
}
...
static void rtl8139_tune_twister (struct net_device *dev, struct dev_private *tp) {
	int linkcase;
	void __iomem *ioaddr = tp->mmio_addr;
...
static inline void rtl8139_thread_iter (struct net_device *dev, struct dev_private *tp, void __iomem *ioaddr) {
	int mii_lpa;
...
static void rtl8139_thread (struct work_struct *work) {
	struct dev_private *tp =
		container_of(work, struct dev_private, thread.work);
...
static inline void rtl8139_tx_clear (struct dev_private *tp) {
	tp->cur_tx = 0;
	tp->dirty_tx = 0;
...
static void rtl8139_tx_interrupt (struct net_device *dev, struct dev_private *tp, void __iomem *ioaddr) {
...
static void rtl8139_rx_err (u32 rx_status, struct net_device *dev, struct dev_private *tp, void __iomem *ioaddr) {
	u8 tmp8;
...
#if RX_BUF_IDX == 3
static inline void wrap_copy(struct sk_buff *skb, const unsigned char *ring, u32 offset, unsigned int size) {
...
static void rtl8139_isr_ack(struct dev_private *tp) {
	void __iomem *ioaddr = tp->mmio_addr;
	u16 status;
...
static int rtl8139_rx(struct net_device *dev, struct dev_private *tp, int budget) {
	void __iomem *ioaddr = tp->mmio_addr;
	int received = 0;
...
static void rtl8139_weird_interrupt (struct net_device *dev, struct dev_private *tp, void __iomem *ioaddr, int status, int link_changed) {
...
static int rtl8139_poll(struct napi_struct *napi, int budget) {
	struct dev_private *tp = container_of(napi, struct dev_private, napi);
	...
	if (work_done < budget) {
		...
		RTL_W16_F(IntrMask, dev_intr_mask);
		spin_unlock_irqrestore(&tp->lock, flags);
	}
...
static irqreturn_t rtl8139_interrupt (int irq, void *dev_instance) {
	...
	/* shared irq? */
	if (unlikely((status & dev_intr_mask) == 0))
		goto out;
	...
	if (status & RxAckBits){
		if (napi_schedule_prep(&tp->napi)) {
			RTL_W16_F (IntrMask, dev_norx_intr_mask);
